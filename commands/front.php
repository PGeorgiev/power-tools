 ...
 */
class PowerTools_Front_Command {
    ...
    public function page_callback() {
    	...
    	if ( absint( $response->stdout ) > 0 ) {
    		...
    		$resp = WP_CLI::launch_self( 'post create', array(), array( 'post_title' => 'Front Page', 'post_content' => $page_content, 'post_type' => 'page', 'post_status' => 'publish', 'porcelain' => true ), false, true );
    		$page_id = $resp->stdout;
    	}
    	if ( absint( $page_id ) > 0 ) {
		...
		if ( absint( $response->stdout ) > 0 ) {
    		...
    		$resp = WP_CLI::launch_self( 'post create', array(), array( 'post_title' => 'Blog', 'post_content' => $page_content, 'post_type' => 'page', 'post_status' => 'publish', 'porcelain' => true ), false, true );
    		$page_id = $resp->stdout;
    	}
...
WP_CLI::add_command( 'pt front', 'PowerTools_Front_Command' );
